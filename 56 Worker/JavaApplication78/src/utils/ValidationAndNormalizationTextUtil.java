...
public class ValidationAndNormalizationTextUtil {
    ...
    public static double getDouble(String msg, String error, double min, double max) {
        while (true) {
            double input = Double.parseDouble(getStringByRegex(msg, "[0-9]+", error));
            if (input < min || input > max) {
                System.err.println("Out of range!");
            } else {
                return input;
    ...
    public static boolean checkYN() {
        String answer = getStringByRegex("Do you want to continue? (Y/N): ", "[a-zA-Z]", "Please only enter y/Y for yes or n/N for no").toLowerCase();
