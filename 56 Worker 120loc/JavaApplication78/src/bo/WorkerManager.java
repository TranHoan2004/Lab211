...
public class WorkerManager {
    ...
    private final List<Worker> listOfWorker;
    private final List<History> listOfHistory;
    ...
    public void addWorker(Worker work) throws Exception {
        if (isExist(work.getId())) throw new Exception("Id is existed");
        ...
        for (Worker worker : listOfWorker) {
            if (worker.getId().equalsIgnoreCase(id)) {
                if (status == Status.UP) {
                    ...
                    if (amount > worker.getSalary()) throw new Exception("Amount is larger than salary");
                    worker.setSalary(worker.getSalary() - amount);
                }
                return worker;
            }
...
//        return false;
        return listOfWorker
                .stream()
                .anyMatch(person -> person.getId().equalsIgnoreCase(id));
    }

    public List<History> getHistory() {
        return new ArrayList<>(listOfHistory); // tra ve mot danh sach moi, ban sao cua danh sach cu
    }
